...
class CockroachDBSchemaManager extends PostgreSQLSchemaManager
{
    ...
    protected function _getPortableTableColumnDefinition($tableColumn)
    {
        ...
        if (
            ...
            && preg_match("/^(?:nextval\('(.*)'(::.*)?\)|unique_rowid\(\))$/", $tableColumn['default'], $matches) === 1
        ) {
        ...
        if (
            $tableColumn['default'] !== null
            && preg_match("('([^']+)'::)", $tableColumn['default'], $match) === 1
        ) {
    ...
    protected function buildQueryConditions($tableName): array
    {
        $conditions = [];
